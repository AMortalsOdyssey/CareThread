...
真实照片使用独立 R 组，仅保存在
`Benchmarks/OCRBench/testset/real/` 的开发者本机目录。该目录默认整目录
Git ignore；样本 ID 与文件名必须是 `r001` 这类无语义编号，图片、人工校对
文本、清单和 OCR 原文一律不得入库。续页只标注实际存在的字段，避免把没有
页眉的报告续页误判为四项业务字段全部失败。
...
真实集命令复用同一 Vision、RapidOCR 与发布版 `ExtractionEngine`，但所有
含 OCR 原文和字段原值的中间文件只进入权限隔离的 `/tmp` 临时目录，退出即
清理。仓库内的 `results/real/results.json` 与 `RESULTS.md` 仅包含图片/
参考文本 SHA-256、字符与字段计数、CER、延迟和 v2 判定，不包含文件名、
路径、OCR 文本、医院、日期或指标原值。现有 40 页真实照片已完成 Vision
离线可运行性、性能及单人一次字段判定；打印件字段合计 100/133（75.19%）。
本批没有手写为主的页面，也没有逐字双录参考文本，因此不伪造手写字段准确率
或真实集 CER；发布结论仍唯一为 Apple Vision。
...
真实照片 R 组继续执行同一固定门槛，不以相对改善率或字段提升另造替代规则。
本轮 40 页均为打印为主；单人一次、非双盲的字段人工判定为 100/133
（75.19%），但没有逐字双录真值和手写为主样本，因而不能计算 R 组 CER 或
手写字段准确率，也不能执行真实集换引擎门槛。完整证据与边界见
`docs/OCR_REAL_PHOTO_BASELINE.md`。
...
- App 必须保留原件、机器层与人工确认层，大字版继续采用“拍了先存、家人后补”。
...
2. 新模型在本机 R 组真实照片回归中同时达到下述固定换引擎门槛。
...
对同一批本机真实照片和同一份人工校对文本，Apple Vision 是基线，挑战者
必须同时满足：

```text
手写 CER 改善 = Vision 手写 CER - 挑战者手写 CER >= 10 个百分点
打印 CER 恶化 = 挑战者打印 CER - Vision 打印 CER <= 1 个百分点
```

两项按绝对百分点计算并必须同时通过；只对 §5 加权总分胜者执行，不能用字段
命中提升替代任一 CER 门槛。字段命中仍按打印/手写分列，分母只统计该页实际
标注的日期、医院、类型、关键指标；没有页眉的续页不会按缺失四项处罚。总分
胜者未同时通过时，唯一结论是维持 Apple Vision，不把选择留给用户。

R 组判断决定是否重启引擎集成；P/H/D 虚构基准继续用于可复现回归、性能
对照和防止已知能力倒退。候选仍须同时满足许可证、完全离线、模型随包、
包体和真机内存红线，GPL/AGPL 继续直接出局。
