...
路径、OCR 文本、医院、日期或指标原值。当前尚未收到真实照片，因而不伪造
R 组分数；发布结论仍唯一为 Apple Vision。
...
上述“改善 10 个百分点”在 Vision 合成手写 CER 仅 3.92% 时数学上不可达，
不再作为真实照片重启门槛。R 组权威规则改为 §7 的“重启条件 v2”；它不会
反向改写或美化这次合成集原始结果。
...
2. 新模型在本机 R 组真实照片回归中同时达到下述“重启条件 v2”。
...
```text
CER 相对改善率 = (Vision CER - 挑战者 CER) / Vision CER >= 50%
字段命中率提升 = 挑战者字段命中率 - Vision 字段命中率 >= 5pp
```

Vision CER 为 0 时，相对改善率定义为 0，避免除零并禁止错误放行。字段
命中分母只统计该页实际标注的日期、医院、类型、关键指标；没有页眉的续页
不会按缺失四项处罚。两项门槛必须同时通过：无人通过时唯一结论是维持
Apple Vision；有候选通过时，按 CER 更低、字段命中更高、P95 更低的固定
顺序选出唯一胜者，不把选择留给用户。
