...
> 项目仍在一期开发中。App 不提供诊断或治疗建议；当前代码不连接服务器、不上传病历，也不包含真实医疗测试数据。

## 一期功能地图

| 能力 | 目标 | 状态 |
|---|---|---|
| 多成员管理 | 最多 20 位家人，数据、原件、提醒、导出和对比严格隔离 | 开发中 |
| 本地 OCR | Apple Vision 离线识别病历、手写处方、化验单和发票 | 引擎完成 |
| 姓名归属闸门 | OCR 姓名错配时阻止直接入库；仅经明确二次确认可覆盖并留审计 | 开发中 |
| 病程时间线 | 日期、病种/类型、医院、医生、事件年龄组合筛选与稳定分页 | 开发中 |
| 多报告/多页导入 | 一次导入多份报告和多页/多截图，用户分组为权威、OCR 辅助建议 | 开发中 |
| 原件 Vault | 不可变原件、高清查看、完整性校验、安全副本与恢复 | 开发中 |
| 提醒 | 用药/复查本地通知；可选、独立授权的系统日历写入 | 计划中 |
| 导出与分享 | 单成员按 1/6 月、1/2/5/10 年或全部生成 PDF，经系统分享面板发送 | 计划中 |
| 本地对比 | 同成员两个阶段的指标和事实对比，不做诊断推断 | 计划中 |
| 附近同步/换机 | 两台 iPhone 近场加密传输，支持单成员或全部成员 | 计划中 |
| 手动编辑与修订 | 所有业务内容可更正、查看修订并撤销；原件/OCR 证据不被覆盖 | 开发中 |
| 就诊准备 | 一页摘要、问题清单、就诊笔记、重要置顶和补药提醒 | 计划中 |
| 双模式 | 标准版五槽导航与老人版三槽导航，功能同等可达 | 骨架完成 |
...
`verify.sh` 负责构建、单元/UI 测试和红线扫描；`acceptance.sh` 是最终交付门禁，开发中会随 M0–M9 逐步扩充。OCR 基准只使用程序生成的虚构资料。
...
| M3–M7 | 多成员、录入、时间线、用药、提醒、导出、对比、附近同步 | 进行中 |
| M8–M9 | 打磨、无障碍、截图、上架材料与总验收 | 未开始 |

当前证据见 [实施进度](docs/PROGRESS.md) 和 [实施日志](docs/IMPLEMENTATION_LOG.md)。
...
- 开源、版权与可持续发展策略：主开发验收后生成，当前由需求追踪表 `Q01–Q10` 管理
...
当前代码以 [MIT License](LICENSE) 开源。MIT 允许使用、修改、分发、再许可和商业使用，但必须保留版权与许可声明；CareThread 名称、商标、App 图标和商店素材的权利边界将在主开发验收后的可持续发展报告中单独明确。
