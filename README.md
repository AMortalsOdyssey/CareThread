...
CareThread 是一款纯原生 iOS 本地病程资料整理工具。它把病历、处方、化验单、发票、用药和复查事项整理成按成员隔离的时间线，提供标准版与长辈版两种完整交互模式。

> 一期 M0–M9、Review 整改与第二轮新增能力均已完成；搜索权限防回归源码 `c96e0e2` 与截图证据 `c6c8801` 的最终验收全绿。App 不提供诊断或治疗建议；当前代码不连接互联网服务器、不上传病历，也不包含真实医疗测试数据。仍需真机完成的系统边界见 `docs/PROGRESS.md` 末尾。
...
| 问我的资料 | 结构化事实查询 + BM25 本地检索；标准版可输入，长辈版提供 4 个大按钮 | 完成 |
...
| 重复资料拦截 | 原件 SHA 硬拦截；pHash/Vision 指纹入库一次并持久化，重拍/裁边与多页 OCR 重叠软提示，确认前复扫 | 完成 |
...
| 双模式 | 标准版五槽导航与长辈版三槽导航，关键能力完整可达 | 完成 |
| 外观主题 | 跟随系统、浅色、深色三档，标准版与长辈版共享并持久化 | 完成 |

### 界面预览

| 资料记录 | 家人档案 | 长辈版今天 | 附近同步 |
|---|---|---|---|
| ![资料记录](docs/screenshots/05-records-light.png) | ![家人档案](docs/screenshots/19-member-management-light.png) | ![长辈版今天](docs/screenshots/15-elder-today-light.png) | ![附近同步](docs/screenshots/22-nearby-sync-light.png) |
...
当前最终全量基线为 Xcode 26.6、iPhone 16 / iOS 26.5：721/721 单元与集成测试、62/62 XCUITest，合计 783/783，失败 0、跳过 0。`Scripts/acceptance.sh` 退出 0，M0–M9、23/23 边界、法律三处同源、权限逐字、零联网、Network.framework 仅限 NearbyTransfer、依赖白名单、46 张截图与走查证据全部 PASS。记录页搜索使用本机 SwiftData；专项 UI 回归会实际输入关键词并监视 App 与 SpringBoard，确保搜索不出现本地网络权限提示。真机已完成签名、安装与冷启动，XCTest 真机远程会话仍受 Xcode/CoreDevice 传输层阻断；其他残留仅表示模拟器不能替代真机，详见[阻塞与边界](docs/BLOCKERS.md)。

## 里程碑

| 里程碑 | 内容 | 状态 |
|---|---|---|
| M0 | 工程骨架、设计 token、标准版/长辈版根导航 | 完成 |
...
| M8–M9 | 备份、应用锁、完整长辈版、无障碍、截图与总验收 | 完成 |
